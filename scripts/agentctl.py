...
def build_parser() -> argparse.ArgumentParser:
    ...
    inject.add_argument(
        ...
        type=float,
        metavar=("X1", "Y1", "X2", "Y2"),
        help="Normalized bbox; omit to send an explicit invisible state",
    )
    ...
    try:
        for sequence in range(args.repeat):
            observation = TargetObservation(
                ...
                bbox=bbox,
                confidence=args.confidence if bbox is not None else 0.0,
            )
                ...
                if key != last_key:
                    ...
                    if args.json:
                        ...
                        print(
                            ...
                            f"visible={observation.visible} bbox={observation.bbox}"
                        )
                    last_key = key
            now = time.monotonic()
