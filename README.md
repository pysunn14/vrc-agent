...
프로젝트의 자체 코드는 [GNU AGPL-3.0](LICENSE)으로 배포합니다. 외부 코드·모델·음성 샘플에는 각각의 라이선스가 적용됩니다. [외부 구성요소 안내](THIRD_PARTY_NOTICES.md)를 확인하세요.

구매 아바타와 아바타에서 추출한 골격·개인 교정 데이터는 배포하지 않습니다. 사용자가 이용 권한을 가진 아바타에서 직접 준비해야 합니다. 저장소 라이선스가 외부 아바타나 모델의 이용 권한을 부여하지는 않습니다.
